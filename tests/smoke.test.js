...
const oldWalk = { vx: Game.player.vx, onGround: Game.player.onGround, climbing: Game.player.climbing };
Game.player.vx = 120; Game.player.onGround = true; Game.player.climbing = null;
Sprites.drawPlayer(ctxStub_, Game.player, 0.5);
Game.player.vx = oldWalk.vx; Game.player.onGround = oldWalk.onGround; Game.player.climbing = oldWalk.climbing;
check('走路角色 sheet 載入', __loadedImages.includes('assets/sprites/player/hero-warrior-walk-sheet.png'));
Sprites.drawPet(ctxStub_, Game.pet, 0.5);
check('寵物 PNG 載入', __loadedImages.includes('assets/sprites/pet/pet-default.png'));
Sprites.drawPet(ctxStub_, { x: 0, y: 0, type: 'pig', animT: 0, attackAnim: 0, facing: 1 }, 0.5);
Sprites.drawPet(ctxStub_, { x: 0, y: 0, type: 'fox', animT: 0, attackAnim: 0, facing: 1 }, 0.5);
check('寵物商店 PNG 載入', ['assets/sprites/pet/pet-pig.png', 'assets/sprites/pet/pet-fox.png'].every((src) => __loadedImages.includes(src)));
...
Sprites._drawWeaponAsset(ctxStub_, { equips: { weapon: 'battleAxe' } }, 0.5, 0.5, 80, 84);
check('wtype swing FX 載入', __loadedImages.includes('assets/sprites/fx/swing/axe-sheet.png'));
new Projectile({ from: 'player', x: 0, y: 0, dir: 1, speed: 0, style: 'bullet' }).draw(ctxStub_, 0.5);
check('投射物 PNG 載入', __loadedImages.includes('assets/sprites/fx/proj/bullet.png'));
UI._skillIcon(ctxStub_, 'dragonStrike', 0, 0, 64);
check('技能 icon PNG 載入', __loadedImages.includes('assets/ui/skills/dragonStrike.png'));
